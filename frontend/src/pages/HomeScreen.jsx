...
// This mock data is only used by the static SwapRequestsView.
const swapRequests = [
...
// --- CORRECTED UserCard component to match the backend data schema ---
const UserCard = ({ user }) => (
    <Link to={`/users/${user._id}`} className="bg-white border rounded-lg p-6 flex flex-col justify-between shadow-sm hover:shadow-lg transition-shadow duration-300 hover:border-blue-500">
        <div>
            <div className="flex justify-between items-start">
                ...
                <div className="flex items-center space-x-2">
                    <span className={`text-xs font-semibold ${user.availability === 'available' ? 'text-green-700 bg-green-100' : 'text-yellow-700 bg-yellow-100'} px-2 py-1 rounded-full capitalize`}>
                        {user.availability}
                    </span>
                </div>
            </div>
            ...
            <div className="mt-4 space-y-4">
                <div>
                    ...
                    <div className="flex flex-wrap gap-2 mt-2">
                        {(user.skillsOffered || []).map(skill => <span key={skill} className="bg-blue-100 text-blue-800 text-xs font-semibold px-2.5 py-0.5 rounded-full">{skill}</span>)}
                    </div>
                </div>
                <div>
                    <h4 className="text-sm font-semibold text-gray-500">Looking for</h4>
                    <div className="flex flex-wrap gap-2 mt-2">
                        {(user.skillsWanted || []).map(skill => <span key={skill} className="bg-green-100 text-green-800 text-xs font-semibold px-2.5 py-0.5 rounded-full">{skill}</span>)}
                    </div>
                </div>
            </div>
        </div>
        <div className="mt-6">
            ...
            <div className="w-full mt-2 text-center bg-gray-800 text-white py-2 rounded-lg hover:bg-gray-700 font-semibold transition-colors">
                View Profile
            </div>
        </div>
    </Link>
...
const RequestCard = ({ request }) => {
    const statusInfo = {
        Pending: { badge: 'bg-yellow-100 text-yellow-800', text: 'Pending', actions: (<div className="flex items-center space-x-3 mt-4"><button className="bg-green-500 text-white font-bold py-2 px-4 rounded-lg hover:bg-green-600 transition-colors">Accept</button><button className="bg-red-500 text-white font-bold py-2 px-4 rounded-lg hover:bg-red-600 transition-colors">Reject</button></div>) },
        Accepted: { badge: 'bg-green-100 text-green-800', text: 'Accepted', actions: null },
        Rejected: { badge: 'bg-red-100 text-red-800', text: 'Rejected', actions: null },
    };
      ...
      <div className="bg-white border border-gray-200 rounded-xl p-6 shadow-sm flex items-center justify-between hover:shadow-lg hover:border-gray-300 transition-all duration-300">
        <div className="flex items-center space-x-6">
          ...
          <div>
            ...
            <div className="flex items-center text-sm mb-3"><span className="text-gray-500 w-28 font-medium">Skills Offered:</span><div className="flex flex-wrap gap-2">{request.skillsOffered.map(skill => <span key={skill} className="bg-blue-100 text-blue-800 text-sm font-semibold px-3 py-1 rounded-full">{skill}</span>)}</div></div>
            <div className="flex items-center text-sm"><span className="text-gray-500 w-28 font-medium">Skill Wanted:</span><div className="flex flex-wrap gap-2">{request.skillsWanted.map(skill => <span key={skill} className="bg-green-100 text-green-800 text-sm font-semibold px-3 py-1 rounded-full">{skill}</span>)}</div></div>
          </div>
        </div>
        <div className="text-right"><span className={`px-3 py-1 text-sm rounded-full font-semibold ${currentStatus.badge}`}>{currentStatus.text}</span>{currentStatus.actions}</div>
      </div>
    );
};
...
// --- CORRECTED DiscoverView component with proper data fetching ---
const DiscoverView = () => {
    ...
    if (loading) {
        return <div className="flex justify-center items-center h-96"><Loader2 className="w-12 h-12 animate-spin text-blue-500"/></div>;
    }

    if (error) {
        return <div className="p-8 text-center text-red-500 font-semibold">Error loading users: {error}</div>;
    }
    ...
    return (
        <>
            ...
            <div className="p-8 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                {/* Use the unique user._id for the key prop */}
